...
/** One goal's loop. Everything below closes over its own `goal`. */
export interface ConvergenceHandle {
  start(req: ConvergeRequest): Promise<{ goalId: string }>;
  progress(): ConvergeProgress | null;
  settled(): Promise<ConvergeProgress | null>;
  cancel(): Promise<void>;
  accept(label: string): Promise<void>;
  reject(label: string, note: string): Promise<void>;
}

function createConvergence(): ConvergenceHandle {
  let goal: ActiveGoal | null = null;
    let settle: Array<(p: ConvergeProgress) => void> = [];

  function getConvergence(): ConvergeProgress | null {
  ...
  function publish(): void {
    if (!goal) return;
    broadcastAll();
    if (!goal.progress.running) {
      const waiters = settle;
      settle = [];
      for (const w of waiters) w(goal.progress);
    }
  }
  ...
  async function startConvergence(req: ConvergeRequest): Promise<{ goalId: string }> {
    if (goal) throw new Error("this convergence has already been started");
   ...
   * goals.
   */
  function waitForConvergenceSettled(): Promise<ConvergeProgress | null> {
    const g = goal;
    if (!g) return Promise.resolve(null);
    if (!g.progress.running) return Promise.resolve(g.progress);
    return new Promise((resolve) => settle.push(resolve));
  }

  async function cancelConvergence(): Promise<void> {
    if (!goal) return;
    goal.cancelled = true;
  ...
  async function acceptTarget(label: string): Promise<void> {
    const g = goal;
    const state = targetState(label);
   ...
   */
  async function rejectTarget(label: string, note: string): Promise<void> {
    const g = goal;
    const state = targetState(label);
  ...
  return {
    start: startConvergence,
    progress: getConvergence,
    settled: waitForConvergenceSettled,
    cancel: cancelConvergence,
    accept: acceptTarget,
    reject: rejectTarget,
  };
}
...
// ── Registry ─────────────────────────────────────────────────────────────────
//
// Several goals converge at once when the fleet allows; each is its own
// instance. The single-goal API is kept for callers that mean "the latest".

const instances = new Map<string, ConvergenceHandle>();
let latestId: string | null = null;

function broadcastAll(): void {
  broadcastToRenderers("parity_converge", JSON.parse(JSON.stringify(listConvergences())));
}

export function listConvergences(): ConvergeProgress[] {
  return [...instances.values()].map((h) => h.progress()).filter((p): p is ConvergeProgress => p !== null);
}

export async function startConvergence(req: ConvergeRequest): Promise<{ goalId: string }> {
  const handle = createConvergence();
  const started = await handle.start(req);
  instances.set(started.goalId, handle);
  latestId = started.goalId;
  // Finished goals stay visible until the list grows long; then the oldest go.
  const done = [...instances.entries()].filter(([, h]) => h.progress()?.running === false);
  for (const [id] of done.slice(0, Math.max(0, done.length - 8))) instances.delete(id);
  broadcastAll();
  return started;
}

function pick(goalId?: string): ConvergenceHandle | undefined {
  return instances.get(goalId ?? latestId ?? "");
}

export function getConvergence(goalId?: string): ConvergeProgress | null {
  return pick(goalId)?.progress() ?? null;
}

export function waitForConvergenceSettled(goalId?: string): Promise<ConvergeProgress | null> {
  return pick(goalId)?.settled() ?? Promise.resolve(null);
}

export async function cancelConvergence(goalId?: string): Promise<void> {
  if (goalId) return pick(goalId)?.cancel();
  await Promise.all([...instances.values()].map((h) => h.cancel()));
}

export async function acceptTarget(label: string, goalId?: string): Promise<void> {
  await pick(goalId)?.accept(label);
}

export async function rejectTarget(label: string, note: string, goalId?: string): Promise<void> {
  await pick(goalId)?.reject(label, note);
}

...
export async function buildAgentSystemPrompt(device: DeviceInfo | null): Promise<string> {
  ...
  lines.push(
    ...
    "identical to a user.",
    "",
    "- `snap_parity` — compare what every device is showing RIGHT NOW. No flow.",
    "  Reach for this first: navigate the devices to the screen in question, then",
    "  snap. Repeated snaps accumulate into one session, a row per screen.",
    "- `reset_parity_session` — drop the captured screens and start over.",
    "- `start_parity_run` — walk one flow through a reference device and several",
    "  target devices at once. Use when a scripted journey already exists, or when",
    "  the comparison needs to be repeatable in CI.",
    "- `get_parity_run` — read progress, then the checkpoint x target grid.",
    "",
    "For the flow path, the flow needs `- checkpoint: <name>` steps marking the",
    "screens worth comparing; outside a parity run they are no-ops, so a flow",
    "carrying them still runs normally. Do not write a flow just to reach one",
    "screen — drive the devices and snap instead.",
    "",
  ...
  if (cases.length) {
    ...
    lines.push(
      ...
      "    properties:",
      "      testCaseId: \"MC-12\"",
      "      priority: \"High\"        # the case's Qase priority; link_case_flow fills it in",
      "",
      ...
      "and `link_case_flow` to write that property into a flow you wrote.",

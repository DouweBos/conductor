...
import dev.houwert.conductor.location.FusedLocationProvider
import dev.houwert.conductor.location.LocationManagerProvider
import dev.houwert.conductor.location.MockLocationProvider
import dev.houwert.conductor.location.PlayServices
import dev.houwert.conductor.screenshot.ScreenshotException
import dev.houwert.conductor.screenshot.ScreenshotService
...
import conductor_android.addMediaResponse
import conductor_android.checkWindowUpdatingResponse
import conductor_android.deviceInfo
import conductor_android.emptyResponse
import conductor_android.eraseAllTextResponse
import conductor_android.inputTextResponse
import conductor_android.launchAppResponse
import conductor_android.screenshotResponse
import conductor_android.setLocationResponse
...
class ConductorDriverService {
    ...
    fun grpcServer() {
        ...
        val port = InstrumentationRegistry.getArguments().getString("port", "3763").toInt()
...
class Service(
    ...
    override fun launchApp(
        request: ConductorAndroid.LaunchAppRequest,
        responseObserver: StreamObserver<ConductorAndroid.LaunchAppResponse>
    ) {
        try {
            ...
            if (intent == null) {
                Log.e("Conductor", "No launcher intent found for package ${request.packageName}")
        ...
        try {
            ...
            val ms = measureTimeMillis {
                if (toastAccessibilityListener.getToastAccessibilityNode() != null && !toastAccessibilityListener.isTimedOut()) {
                    Log.d("Conductor", "Requesting view hierarchy with toast")
                    ViewHierarchy.dump(
                        uiDevice,
                        uiAutomation,
                        stream,
    ...
    override fun addMedia(responseObserver: StreamObserver<ConductorAndroid.AddMediaResponse>): StreamObserver<ConductorAndroid.AddMediaRequest> {
        return object : StreamObserver<ConductorAndroid.AddMediaRequest> {
            ...
            override fun onNext(value: ConductorAndroid.AddMediaRequest) {
    ...
    override fun eraseAllText(
        request: ConductorAndroid.EraseAllTextRequest,
        responseObserver: StreamObserver<ConductorAndroid.EraseAllTextResponse>
    ) {
        try {
            val charactersToErase = request.charactersToErase
            Log.d("Conductor", "Erasing text $charactersToErase")
    ...
    override fun inputText(
        request: ConductorAndroid.InputTextRequest,
        responseObserver: StreamObserver<ConductorAndroid.InputTextResponse>
    ) {
        try {
            Log.d("Conductor", "Inputting text")
    ...
    override fun screenshot(
        request: ConductorAndroid.ScreenshotRequest,
        responseObserver: StreamObserver<ConductorAndroid.ScreenshotResponse>
    ) {
        try {
    ...
    override fun isWindowUpdating(
        request: ConductorAndroid.CheckWindowUpdatingRequest,
        responseObserver: StreamObserver<ConductorAndroid.CheckWindowUpdatingResponse>
    ) {
        try {
    ...
    override fun disableLocationUpdates(
        request: ConductorAndroid.EmptyRequest,
        responseObserver: StreamObserver<ConductorAndroid.EmptyResponse>
    ) {
        try {
    ...
    override fun setLocation(
        request: ConductorAndroid.SetLocationRequest,
        responseObserver: StreamObserver<ConductorAndroid.SetLocationResponse>
    ) {
        try {
    ...
    private fun setText(text: String) {
        for (element in text) {
            Log.d("Conductor", element.code.toString())
            when (element.code) {
                in 48..57 -> {
                    /** 0~9 **/

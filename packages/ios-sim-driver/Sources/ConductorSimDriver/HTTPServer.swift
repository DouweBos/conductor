...
// Mirrors the layout of packages/ios-dylib/.../HTTPServer.swift so the wire
// behavior is identical; just the host (macOS) and the route handlers differ.
//
// One request per connection. Conductor only has one outstanding command in
// flight per session, so the simplicity is fine.
//
import Foundation
import Network
...
final class HTTPServer {
    ...
    private let queue = DispatchQueue(label: "dev.houwert.conductor-sim-driver.http")
    ...
    func start() throws {
        ...
        // Bind explicitly to 127.0.0.1 — never wildcard. We don't set
        // acceptLocalOnly because the dylib counterpart hit issues with it.

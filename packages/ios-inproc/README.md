...
| `GET /set?id=&key=&value=` | `native-set` | **live-edit** a whitelisted property (alpha, hidden, backgroundColor, tintColor, cornerRadius, borderWidth, borderColor, frame, text, textColor). `text`/`textColor` also work on RN Fabric text views (`attributedText`-based) |
| `GET /props?id=` | `native-props` | React Native Fabric props: typed `ViewProps` (from the mounted view) + `rawProps` (raw JS prop bag). Non-Fabric views return `isFabric:false` |

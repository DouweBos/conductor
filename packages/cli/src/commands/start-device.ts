...
async function startIOS(osVersion: string | undefined, opts: OutputOptions): Promise<number> {
    ...
    for (const sim of sims) {
      ...
      if (sim.isAvailable && sim.state === 'Booted') {
        if (!osVersion || runtime.includes(osVersion)) {
          printSuccess(`Simulator already booted: ${sim.name} (${sim.udid})`, opts);
          return 0;
        }
      }
    }
  ...
  printSuccess(`Booted: ${device.name} (${device.udid})`, opts);
  return 0;
}
...
export async function startDevice(
  ...
  opts: OutputOptions,
  flags: { osVersion?: string; avd?: string }
): Promise<number> {
  if (!platform) {

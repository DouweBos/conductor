...
export const SET_HELP = `  native-set <id> <key> <value>        Live-edit a view property (alpha, hidden, backgroundColor, tintColor, cornerRadius, borderWidth, borderColor, frame, text, textColor). text/textColor also apply to RN Fabric text`;
export const PROPS_HELP = `  native-props <id>                    React Native Fabric props: typed ViewProps + raw JS prop bag (Fabric views only)`;
...
export async function nativeProps(
  id: string | undefined,
  opts: OutputOptions = {},
  sessionName = 'default'
): Promise<number> {
  if (!id) {
    printError('native-props needs a view id (from native-inspect)', opts);
    return 1;
  }
